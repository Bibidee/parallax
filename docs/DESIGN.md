...
The model returns a bounded object: `spec_match`, `visual_change`, `evidence_support`, `evidence_quality`, `risk`, `confidence`, and `rationale`. Equivalence requires exact agreement on all five material semantic fields (`spec_match`, `visual_change`, `evidence_support`, `evidence_quality`, and `risk`). Confidence may differ, but both observations must be on the same side of the deterministic 75-point approval threshold; rationale is explanatory and may differ. Material disagreement cannot be accepted merely because both results are blocked, and it cannot trigger a punitive worker-bond settlement. Approval still requires the complete safe tuple and confidence at least 75 in each observation. Technical/artifact and model failures are retryable and non-punitive; malformed outputs never approve.
